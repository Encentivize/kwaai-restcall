    ...
    function tryClient(){
        ...
        try{
            request[options.verb](requestOptions,function(error, response, body){
                if (error){
                    if (retryCnt>0) {
                        console.log("retrying api call :" + retryCnt + ":" +  options.verb + ":" + requestOptions.url + ":" + JSON.stringify(error));
            ...
            if (retryCnt>0) {
                console.log("retrying api call :" + retryCnt + ":" + options.verb + ":" + requestOptions.url +  ":" + JSON.stringify(exp));
                setTimeout(tryClient, 5000);
            }else{
